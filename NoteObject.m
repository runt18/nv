...
- (BOOL)writeUsingCurrentFileFormat {
    ...
    if (formattedData) {
		...
		if (NTVStorageFormatPlainText == formatID) {
...
- (OSStatus)writeFileDatesAndUpdateTrackingInfo {
	if (NTVStorageFormatDatabase == currentFormatID) return noErr;
...
- (OSStatus)writeCurrentFileEncodingToFSRef:(FSRef*)fsRef {
	...
	OSStatus err = noErr;
	if ((err = FSRefMakePath(fsRef, [pathData mutableBytes], (UInt32)[pathData length])) == noErr) {
...
- (BOOL)updateFromCatalogEntry:(NoteCatalogEntry*)catEntry {
	...
	NSMutableData *pathData = [NSMutableData dataWithLength:4 * 1024];
	if (FSRefMakePath(noteFileRefInit(self), [pathData mutableBytes], (UInt32)[pathData length]) == noErr) {
...
-(void)odbEditor:(ODBEditor *)editor didModifyFile:(NSString *)path newFileLocation:(NSString *)newPath  context:(NSDictionary *)context {
	...
	if ([self updateFromData:[NSMutableData dataWithContentsOfFile:path options:NSUncachedRead error:NULL] inFormat:NTVStorageFormatPlainText]) {
